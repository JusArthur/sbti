...
const App = () => {
  ...
  const computeResult = () => {
    ...
    const ranked = NORMAL_TYPES.map(type => {
      const vector = type.pattern.replace(/-/g, '').split('').map(l => l === 'L' ? 1 : l === 'M' ? 2 : 3);
    ...
    if (drunkTriggered) {
      ...
      sub = 'Your brain broke the entire personality database. Forced assignment to HHHH.';
      special = true;
    }
        ...
        {screen === 'intro' && (
          <div className="pt-20 text-center">
            ...
            <h1 className="text-7xl font-bold tracking-tighter text-emerald-950 leading-none">
              Discover Your True<br />(Chaotic) Self
            </h1>
        ...
        {screen === 'test' && (
          <div>
            ...
            <div className="space-y-8">
              {visible.map((q, idx) => (
                <div key={q.id} className="bg-white border border-emerald-100 rounded-3xl p-8 shadow-xl">
                    ...
                    {q.options.map((opt, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          <span className="font-bold text-emerald-700 w-6">{code}</span>
        ...
        {screen === 'result' && (() => {
          ...
          return (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              {/* Poster */}
              <div className="lg:col-span-5 bg-white rounded-3xl p-8 shadow-xl flex flex-col">
                <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-emerald-50 to-white rounded-3xl overflow-hidden">
                  {TYPE_IMAGES[type.code] ? (
                    <img src={TYPE_IMAGES[type.code]} alt={type.code} className="max-h-[420px] object-contain" />
                  ) : (
                    <div className="text-8xl font-black text-emerald-200">{type.code}</div>
                  )}
                </div>
                <p className="text-center mt-8 text-emerald-700 text-xl font-medium">{type.intro}</p>
              </div>
              ...
              <div className="lg:col-span-7 space-y-8">
                <div>
                  ...
                  <h1 className="text-6xl font-bold tracking-tighter text-emerald-950">
                    {type.code} — {type.cn}
                  </h1>
                ...
                <p className="text-xl text-emerald-800 leading-relaxed">{result.sub}</p>
                <p className="text-emerald-800 text-[15.5px] leading-relaxed">{type.desc}</p>

                {/* Dimension Breakdown */}
                <div className="bg-white rounded-3xl p-8 shadow-xl">
                  <h3 className="font-semibold text-lg mb-6">15-Dimension Breakdown</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {dimensionOrder.map(dim => {
                      const level = result.levels[dim];
                      return (
                        <div key={dim} className="bg-emerald-50 rounded-2xl p-5">
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className="flex-1 py-6 border border-emerald-200 hover:border-emerald-400 rounded-3xl font-semibold text-lg"
                  >
                    Take Test Again
                  </button>
                  <button
                    onClick={() => setScreen('intro')}
                    className="flex-1 py-6 bg-emerald-700 text-white hover:bg-emerald-800 rounded-3xl font-semibold text-lg"
                  >
                    Back to Home
                  </button>
                </div>
